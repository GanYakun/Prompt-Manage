<template>
  <div class="prompt-detail" v-if="prompt">
    ...
    <div class="detail-title-section">
      <h1 class="detail-main-title">{{ prompt.title }}</h1>
      <div class="detail-meta-info">
        ...
        <div class="meta-item">
          ...
          <span class="meta-value">{{ prompt.version_count || 1 }} 个</span>
        </div>
      </div>
    </div>
    ...
    <div class="detail-actions-section">
      <div class="primary-actions">
        <button class="btn btn-primary" @click="editPrompt">
      ...
      <div class="secondary-actions">
        ...
        <button class="btn btn-outline" @click="exportPrompt">
          <span class="btn-icon">📤</span>导出
        </button>
        <button class="btn btn-error-outline" @click="deletePrompt">
    ...
    <div class="detail-content-section">
      <div class="content-block">
        <div class="content-preview-enhanced">{{ prompt.content }}</div>
      </div>
      ...
      <div v-if="prompt.tags && prompt.tags.length > 0" class="content-block">
...
const prompt = computed(() => appStore.selectedPrompt)

const categoryTags = computed(() => {
  if (!prompt.value?.categories) return []
  
  const tags = []
  Object.entries(prompt.value.categories).forEach(([categoryType, categoryKey]) => {
...
const editPrompt = () => {
  modalStore.show('EditPromptModal', { prompt: prompt.value })
}

const showVersionHistory = () => {
  modalStore.show('VersionHistoryModal', { prompt: prompt.value })
}

const saveAsTemplate = () => {
  modalStore.show('SaveAsTemplateModal', { prompt: prompt.value })
}
...
const exportPrompt = async () => {
  try {
    await window.api.exportPrompt(prompt.value.id)
    notificationsStore.show('导出成功 📤', 'success')
  } catch (error) {
    console.error('导出失败:', error)
    notificationsStore.show('导出失败', 'error')
  }
}

const deletePrompt = async () => {
  const confirmed = await modalStore.confirm(
    `确定要删除想法 "${prompt.value.title}" 吗？`,
    '此操作将删除该想法及其所有版本历史，且不可撤销。'
  )
  
  if (confirmed) {
    try {
      await window.api.deletePrompt(prompt.value.id)
